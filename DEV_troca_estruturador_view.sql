-- CREATE OR REPLACE VIEW sandbox.originacao.troca_estruturador_view AS
...
final AS (
    ...
    log.board_id,
    COALESCE(log.previous_json_map['shkEstruturadorDeNegocio'], log.previous_json_map['shkLiderVertical']) AS previous_user_id,
    us2.name AS previous_user_name,
    COALESCE(log.new_json_map['shkEstruturadorDeNegocio'], log.new_json_map['shkLiderVertical']) AS new_user_id,
    us3.name AS new_user_name,  
    from_utc_timestamp(log.creation_timestamp,'America/Sao_Paulo')::DATE AS data_mudanca,
    log.creation_user_id,
    ...
    ON COALESCE(log.previous_json_map['shkEstruturadorDeNegocio'], log.previous_json_map['shkLiderVertical']) = us2.id
    
    LEFT JOIN raw_views.ocean_workspace_public.users us3
    ON COALESCE(log.new_json_map['shkEstruturadorDeNegocio'], log.new_json_map['shkLiderVertical']) = us3.id
    
    WHERE COALESCE(log.previous_json_map['shkEstruturadorDeNegocio'], log.previous_json_map['shkLiderVertical']) 
        <> COALESCE(log.new_json_map['shkEstruturadorDeNegocio'], log.new_json_map['shkLiderVertical'])
